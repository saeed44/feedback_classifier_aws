...
def build_models(transformer, max_len=40):
...
def make_preds(models, x_test, categ="compliment" ):
    
    weight_paths = {"compliment": "checkpoint_compliment.h5"}
    models[categ].load_weights(weight_paths[categ])
    pred = models[categ].predict(x_test)
...
def predict(df):
    ...
    y_pred = make_preds(models, input_ids, categ="compliment")
...
models = build_models(transformers.TFBertModel.from_pretrained('brt_base_uncase'), max_len=40)  